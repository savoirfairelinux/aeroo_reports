...
class AccountPayment(models.Model):
    ...
    def get_payment_lines(self):
        ...
        partial_reconciles = (
            self.move_line_ids.mapped('matched_credit_ids') |
            self.move_line_ids.mapped('matched_debit_ids')
        )

        full_reconcile = partial_reconciles.mapped('full_reconcile_id')

        if full_reconcile:
            credits = full_reconcile.mapped('partial_reconcile_ids').filtered(
                lambda l: l.credit_move_id not in move_lines)
            debits = full_reconcile.mapped('partial_reconcile_ids').filtered(
                lambda l: l.debit_move_id not in move_lines)

        else:
            # If the reconcile is not complete, attempt to reconcile using
            # the invoices related to the payment.
            credits = (
                self.mapped('invoice_ids.move_id.line_ids.matched_debit_ids')
                .filtered(
                    lambda l: l.debit_move_id in move_lines or
            ...
            credits |= (
                ...
                .filtered(lambda l: l.credit_move_id not in move_lines)
            )

            debits = (
                self.mapped('invoice_ids.move_id.line_ids.matched_credit_ids')
                .filtered(
                    lambda l: l.credit_move_id in move_lines or
        ...
        res = [
            {
                ...
                'name': key[3],
                'amount': amount,
            } for key, amount in lines.items() if key[0]
        ]
