...
class TestCheckPrinting(common.SavepointCase):
    ...
    def create_invoices_and_payment(self, currency):
        ...
        self.partner.property_account_payable_id = account
    ...
    def test_05_get_payment_lines_payment_reconciled_after_post(self):
        self.create_invoices_and_payment(self.company_currency)
        invoices = self.payment.invoice_ids
        account = invoices[0].account_id

        self.payment.write({'invoice_ids': [(5, 0)]})
        self.payment.post()

        payment_line = self.payment.move_line_ids.filtered(
            lambda l: l.account_id == account)

        invoice_lines = invoices.mapped('move_id.line_ids').filtered(
            lambda l: l.account_id == account)
